...
public class ProgressDialog extends Dialog {
    ...
    public static ProgressDialog showHorizontal(Context context, OnInitValues onInitValues) {
        if (context == null) {
            return null;
        }
        ProgressDialog dialog = new ProgressDialog(context, android.R.style.Theme_Translucent_NoTitleBar);
        dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        dialog.setTitle("");
        View view = (LayoutInflater.from(context)).inflate(R.layout.progress_bar_horizontal, null);
        ProgressBar ProgressBar = (android.widget.ProgressBar) view.findViewById(R.id.ProgressBar);
        onInitValues.init(ProgressBar);
        dialog.setContentView(view);
        dialog.setCancelable(false);
        dialog.getWindow().getAttributes().gravity = Gravity.CENTER;
        WindowManager.LayoutParams lp = dialog.getWindow().getAttributes();
