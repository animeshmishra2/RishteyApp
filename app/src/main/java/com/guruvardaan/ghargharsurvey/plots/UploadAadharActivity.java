...
public class UploadAadharActivity extends BaseActivity {

    LinearLayout take_photo, upload_gallery;
    UserDetails userDetails;
    ImageView close_button;
    TextView step_txt, upload_txt, skip_button;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getLayoutInflater().inflate(R.layout.activity_upload_image, frameLayout);
        ...
        upload_gallery.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (checkAndRequestGallery(UploadAadharActivity.this)) {
                    Matisse.from(UploadAadharActivity.this)
        ...
        take_photo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (checkAndRequestPermissions(UploadAadharActivity.this)) {
                    ImagePicker.with(UploadAadharActivity.this)
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        switch (requestCode) {
            case 101:
                if (ContextCompat.checkSelfPermission(UploadAadharActivity.this,
                        Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    Toast.makeText(getApplicationContext(),
                                    "FlagUp Requires Access to Camara.", Toast.LENGTH_SHORT)
                            .show();
                    finish();
                } else if (ContextCompat.checkSelfPermission(UploadAadharActivity.this,
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if (requestCode == 21 && resultCode == RESULT_OK) {
            if (upload_txt.getText().toString().trim().equalsIgnoreCase("Upload PAN")) {
                uploadFiles("pan", Matisse.obtainPathResult(data).get(0));
            ...
            } else if (upload_txt.getText().toString().trim().equalsIgnoreCase("Upload Meeting Pic")) {
                uploadFiles("meeting_pic", Matisse.obtainPathResult(data).get(0));
            }
        }
        if (requestCode == 22 && resultCode == RESULT_OK) {
            if (upload_txt.getText().toString().trim().equalsIgnoreCase("Upload PAN")) {
                uploadFiles("pan", data.getStringExtra(ImagePicker.EXTRA_FILE_PATH));
            } else if (upload_txt.getText().toString().trim().equalsIgnoreCase("Upload Aadhar Front")) {
                uploadFiles("aadhar_front", data.getStringExtra(ImagePicker.EXTRA_FILE_PATH));
            } else if (upload_txt.getText().toString().trim().equalsIgnoreCase("Upload Aadhar Back")) {
                uploadFiles("aadhar_back", data.getStringExtra(ImagePicker.EXTRA_FILE_PATH));
            } else if (upload_txt.getText().toString().trim().equalsIgnoreCase("Upload Meeting Pic")) {
                uploadFiles("meeting_pic", data.getStringExtra(ImagePicker.EXTRA_FILE_PATH));
            }
        }
    }
    ...
    public void uploadFiles(String key, String paths) {
        ...
        UploaderSingle fileUploader = new UploaderSingle();
        System.out.println("Mishur " + getIntent().getExtras().getString("ID"));
        fileUploader.uploadFiles(getIntent().getExtras().getString("ID") + "/", key, new File(paths), new UploaderSingle.FileUploaderCallback() {
            @Override
            public void onError() {
                    ...
                    if (Table.length() > 0) {
                        ...
                        if (object.getString("Column1").equalsIgnoreCase("t")) {
                            if (upload_txt.getText().toString().trim().equalsIgnoreCase("Upload PAN")) {
                                ...
                                step_txt.setText("Steps 3/4");
                                view_1.setBackgroundColor(Color.parseColor("#3700B3"));
                                view_2.setBackgroundColor(Color.parseColor("#3700B3"));
                                view_3.setBackgroundColor(Color.parseColor("#3700B3"));
                                view_4.setBackgroundColor(Color.parseColor("#777777"));
                            } else if (upload_txt.getText().toString().trim().equalsIgnoreCase("Upload Aadhar Back")) {
                                upload_txt.setText("Upload Meeting Pic");
                                step_txt.setText("Steps 4/4");
                                view_1.setBackgroundColor(Color.parseColor("#3700B3"));
                                view_2.setBackgroundColor(Color.parseColor("#3700B3"));
                                view_3.setBackgroundColor(Color.parseColor("#3700B3"));
